...
import type { Card, CreateCardInput, UpdateCardInput } from '@/types/flashcards';
...
export const cardService = {
  ...
  update: (id: string, input: UpdateCardInput) => http<Card>(`/cards/update/${id}`, patchJson(input)),
  remove: (id: string) => http<void>(`/cards/${id}`, { method: 'DELETE' }),
  generateFromAi: (prompt: string, categoryId: string) =>
    http<{ created: number; createdCards: Card[]; skippedWords: string[] }>('/ai/generate', json({ prompt, categoryId })),
};
